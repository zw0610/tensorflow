...
class BaseElasticTest(object):  # pylint: disable=missing-docstring
  ...
  def _restart(self, downtime_secs, task_type, killed_idx, restart_idx):
    """Kills `job` (index: 0) and restarts it after `downtime_secs` with a new name if specified.

    Args:
      downtime_secs: secs before restarting the job.
      job: a string specifying the job to restart.
      new_name: a string specifying the name of the new job.
    """
    self._removeParticipant(downtime_secs, task_type=task_type, restart_idx=killed_idx)
    self._addParticipant(task_type=task_type, restart_idx=restart_idx)

  def _restart_in_thread(self, downtime_secs, task_type, killed_idx, restart_idx):
    def _restart_fn():
  ...
  def _removeParticipant(self, downtime_secs, task_type, killed_idx):
  ...
  def _addParticipant(self, task_type, restart_idx):
    self.assertFalse(context.check_alive(f"/job:{task_type}/replica:0/task:{restart_idx}"))
    self._cluster.start_task(task_type=task_type, task_id=restart_idx)
    while not context.check_alive(f"/job:{task_type}/replica:0/task:{restart_idx}"):
      time.sleep(1)
  ...
  def testWorkerAdditionBetweenFunctions(self):
    ...
    self._addParticipant("worker", self.max_idx_worker + 1)
  ...
  def testWorkerRestoreWithSameName(self):
    ...
    self._restart(downtime_secs=2, task_type="worker", killed_idx=0, restart_idx=0)
  ...
  def testWorkerRestoreWithAnotherName(self):
    ...
    self._restart(downtime_secs=2, task_type="worker", killed_idx=0, restart_idx=self.max_idx_worker + 1)
